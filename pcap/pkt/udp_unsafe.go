...
func NewUdpHdr(p unsafe.Pointer) (*UdpHdr, unsafe.Pointer) {
	...
	udpHead.Source = uint16(C.udphdr_source_ntohs(udpHead.cptr))
	udpHead.Dest = uint16(C.udphdr_dest_ntohs(udpHead.cptr))
	udpHead.Len = uint16(C.udphdr_len_ntohs(udpHead.cptr))
	udpHead.Check = uint16(C.udphdr_check_ntohs(udpHead.cptr))
